...
local function togglevis(self)
    ...
    self.focused = not self.minimized
end
...
function Window:mousereleased( mx, my, button, istouch, presses )
    ...
    self.hidebutt:mousereleased( mx, my, button, istouch, presses )
...
function Window:mousemoved( mx, my, dx, dy, istouch )
    if self.barhold then
        ...
        if self.ypos > love.graphics.getHeight() then self.ypos = love.graphics.getHeight() end
    ...
    self.hidebutt:mousemoved( mx, my, dx, dy, istouch )
...
function Window:new(params)
    ...
    params.hidebutt = button:new({xpos=params.w-bbfset, ypos = -16, w=12, h=12, text = "-", textal = "center", style=params.style, func={togglevis,params}})   
    params.hidebutt.x = params.xpos + params.hidebutt.xpos;params.hidebutt.y = params.ypos + params.hidebutt.ypos
