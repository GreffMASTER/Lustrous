...
local function drawUI( )
    ...
    utils.printfWithShadow( songtext, _GMFont24, 0, 510, 950, "right" )
    ...
    if not hack1 then -- dont update until loaded all
        ...
        if ending then
            ...
            if endtimer <= 0 then
                song.music:stop( )
                if previous_song then previous_song.music:stop() end
                _GMState.change("demo_end",nil,{["ttype"]="fadeout",["color"]={0,0,0},["time"]=1})
            end
        end
        ...
        if song.music:getVolume() < song_max_vol then
            local vol = song.music:getVolume( ) + (dt / 3)
...
function gamestate.draw( )
    ...
    love.graphics.line( 224, 160, 224, _GMState.getHeight()-59 )
    love.graphics.line( 736, 160, 736, _GMState.getHeight()-59 )
    love.graphics.line( 224, _GMState.getHeight()-60, 736, _GMState.getHeight()-60 )
    ...
    if not hack1 then
        ...
        love.graphics.draw(glinegrad, linepos-66, 128+32)
        love.graphics.setLineWidth( 5 )
        love.graphics.line( linepos, 128, linepos, _GMState.getHeight()-60 )
...
function gamestate.buttonpressed( button )
    if not ending and not hack1 then
        ...
        -- pause the game
        if button == 7 and tdone then _GMState.pausegame( ) end
    end
end
